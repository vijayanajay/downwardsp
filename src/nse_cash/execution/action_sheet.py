...
def render_action_sheet(config, result, target_date: Date,
    ...
    for cand, _dec in result.accepted:
        ...
        stop_px = round_to_tick(cand.structural_stop)
        table.add_row(
            cand.symbol,
            ...
            f"₹{t1_px:,.2f}",
            f"₹{t2_px:,.2f}",
            f"₹{stop_px:,.2f} ({-cand.structural_stop_pct * 100:.2f}%)",
            f"{qty} ({t1_qty}/{t2_qty})",
        )
    console.print(table)
...
def _render_playbook() -> None:
    playbook = (
        ...
        "   GTT 1 (T1): target = T1 Target, trigger = Structural Stop.\n"
        "   GTT 2 (T2): target = T2 Target, trigger = Structural Stop.\n"
        "3. Record the fill: nse-cash ledger --record-fill SYMBOL PRICE\n"
        "4. EOD 3:20 PM: nse-cash ledger SYMBOL=PRICE — if GTT 1 filled, "
        "modify GTT 2 stop to Breakeven (Entry).\n"
